  ...
  <div class="approvelist" style="height:100%">
      <view-box>
        ...
        <div class="orderInfo" v-for="(item,index) in good_list" v-if="allCount > 0">
            <div class="orderNo">
            <p><span class="black">{{item.name}}</span></p>
            <p><span class="blue">[{{item.statusName}}]</span></p>
            </div>
            <div class="offerDeta">
                <p>申请人:{{item.user}}</p>
                <p>编号:{{item.sn}}</p>
            ...
            <x-button mini type="warn" @click.native="rover(item)" v-if='privilegeJson.approvalOperate' v-show="item.shenp">审批</x-button>
            <x-button mini type="primary" @click.native="seen(item)"  v-show="item.chakan">查看</x-button>
            </div>
        </div>
