<template>
  <div>
    ...
    <div class="orderInfo" v-for="(item,index) in orderList">
      ...
      <div class="butsubmit">
        <x-button mini :gradients="['#1AA9F8', '#1AA9F8']" @click.native="goOrderDetail(item.id)">订单详情</x-button>
        <x-button mini :gradients="['#1AA9F8', '#1AA9F8']" @click.native="goSendList(item.id)">收货详情</x-button>
