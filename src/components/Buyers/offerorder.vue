<template>
	<div class="shopcar" id="demo04">
		...
		<alert v-model="show" title="提示">
			<emotion is-gif>{{emotion}}</emotion>请选择产品后下单</alert>
		<!-- 未选择提示框 -->
		<!-- 确定下单 -->
		<div v-transfer-dom>
			<popup v-model="show8" position="left" width="100%">
				<div class="position-horizontal-demo">
					...
					<div class="vux-close">
						...
						<x-button mini type="warn" @click.native="bjsubmit">确定下单</x-button>
					</div>
				</div>
			</popup>
		</div>
...
export default {
	...
	methods: {
		...
		bjsubmit() {
			...
			console.log(param);
			axios.post(urlPublic.publicUrl + 'order/add', param).then(response => {
				console.log('提交成功');
				this.$router.push('/buyers/takelist');
			}).
				catch(error => {
					console.log(error);
					alert('网络错误，不能访问');
...
.car-item {
	position: relative;
	width: 93%;
	box-shadow: 0 5px 74px 0 rgba(4, 0, 0, 0.09); /* px */
  	margin: 10px auto;
	border-radius: 3px;
	overflow: hidden;
	padding: 0.4rem;
